...
$(function(){
	...
	var menu = ["home","about","service","case","contactUs"]; //栏目文件名
		...
		_menuA.click(function(){
			...
			if($("#"+menu[aIndex-1]).is(":animated") || isNoMiddle) return false;
				...
				if(aIndex==menuIndex || menuIndex=="0"){
					...
					if(position=="right"){
						$("#"+menu[aIndex-1]).animate({"left":"-100%"},'slow',function(){
							_menuA.eq(aIndex).attr("position","left");
						});
						$("#"+menu[menuIndex-1]).animate({"left":"-3%"},'slow',function(){
							$(this).find(".back").animate({"right":"30px"},'slow');
							$(this).animate({"left":"0%"},'slow',function(){
...
function Execution(i){
	switch (i){
		case 1: homeContent(); break;
		case 2: aboutAnimate();break;
		case 3: serviceAnimate();break;
		case 4: caseAnimate();break;
		case 5: contactUsAnimate();break;
